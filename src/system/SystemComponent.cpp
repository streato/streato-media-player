...
QString SystemComponent::getUserAgent()
{
  ...
  QString userAgent = QString("StreatoMediaPlayer %1 (%2-%3 %4)").arg(Version::GetVersionString()).arg(getPlatformTypeString()).arg(getPlatformArchString()).arg(osVersion);
  return userAgent;
}
...
void SystemComponent::checkForUpdates()
{
  if (SettingsComponent::Get().value(SETTINGS_SECTION_MAIN, "checkForUpdates").toBool()) {
    ...
    QString checkUrl = "https://github.com/streato/streato-media-player/releases/latest";
